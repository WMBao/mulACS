//基于蓝牙通讯与射频识别的智能电子门禁系统设计（Arduino平台源码v1.1）
//By 包伟铭 (SJTU CS)

/*  课程名称：基于安卓开发平台的电子系统综合设计
 *  组　　号：第 6 组
 *  指导老师：张士文
 *  日　　期：2015.7.28(v1.1)
 */
...
//Open the door when received the target data ( Here take "0" as an example! ) 

   if (Serial.available()) {          //Check whether there're some data arrived
    var = (char)Serial.read();        //Read
    if (var == '\40')                  //Check whether the data received is "0"
    {
         lcd.backlight();
         autoSetCursor();
         lcd.print("Welcome!");
...
//Check whether there's a card detected.(All in the function rFID)
//Open the door when the card is permitted.
    rFID();
...
/*
 * Function：rFID
 * Description：Detect and Check if there's a card
 * Input parameters：null
 * return：null
 */
void rFID(void)
{
    ...
    if (status == MI_OK)
    {
        ...
        // Here use cardID1:00000000 and cardID2:FFFFFFFF as an sample 
        uchar* id = serNum;
        if( id[0]==0x00 && id[1]==0x00 && id[2]==0x00 && id[3]==0x00 ) {
            Serial.println("Hello, W.M.Bao!");
            lcd.backlight();
            autoSetCursor();
            lcd.print("Hello, W.M.Bao!");
            servoOpen();
            delay(1000);
            autoSleep();
        } else if(id[0]==0xFF && id[1]==0xFF && id[2]==0xFF && id[3]==0xFF) {
            Serial.println("Hello, superCard!");
            lcd.backlight();
            autoSetCursor();
            lcd.print("Hello, superCard!");
            servoOpen();
            delay(1000);
            autoSleep();
        }else{                  //Invalid card detected
            Serial.println("Invalid attempt!");
            lcd.backlight();
            autoSetCursor();
 ...
 * Description：set cursor automatically
 * Input parameter：null
 * Return：null
 */
 void autoSetCursor(void)
 {
 cursorLine ++;
 lcd.setCursor(0,cursorLine % 2);
 }


/*
 * Function：autoSleep
 * Description：clear and light-off automatically
 * Input parameter：null
 * Return：null
 */
void autoSleep(void)
